...
const agent = "Mozilla/5.0 (Linux; Android 9; Redmi S2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.104 Mobile Safari/537.36";
...
async function PhotoOxy(url, query) {
	return new Promise((resolve, reject)=> {
		...
		axios.get(url, config).then(resp=> {
			...
			axios.post(url, `${query}&token=${encodeURIComponent(token)}&build_server=${encodeURIComponent(build)}&build_server_id=${encodeURIComponent(build_id)}`, config).then(resp=> {
				...
				config.headers.Origin = 'https://photooxy.com';
				config.headers.Referer = url;
				config.headers['X-Requested-With'] = "XMLHttpRequest";
				config.headers.Accept = "*/*";
				config.WithCredentials = true;
				config.headers['Content-Type'] = "application/x-www-form-urlencoded; charset=UTF-8";
				axios.post(`https://photooxy.com/effect/create-image`, form_url, config).then(resp=> {
